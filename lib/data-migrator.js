...
DataMigrator.prototype.setSource = function setSource(source) {
  ...
  if ( _.isNil(source) || ! _.isObject(source) ) throw new TypeError('source must be a plain object and not null');
...
DataMigrator.prototype.setTarget = function setTarget(target) {
  ...
  if ( _.isNil(target) || ! _.isObject(target) ) throw new TypeError('target must be a plain object and not null');
...
DataMigrator.prototype.addPath = function addPath(params) {
  ...
  for (let a = 0; a < params.length; a++) {
    ...
    // If the from or to are not functions convert them to one (aka binding them to the _.get)
    if ( ! _.isFunction(pathMap.from) ) pathMap.from = _.get.bind(_, this._source, pathMap.from);
    if ( ! _.isFunction(pathMap.to) ) {
      pathMap.toPathKey = pathMap.to;
      pathMap.to = (value) => { return _.set.bind(_, this._target, pathMap.toPathKey, value)(); };
    }
...
DataMigrator.prototype._runPath = function _runPath(fromPath) {
  ...
  for (let x = 0; x < toPaths.length; x++) {
    ...
    for (let y = 0; y < useFromValue.length; y++) {
      ...
      if ( pathMap.isToArray && _.has(pathMap, 'toValue') ) {
